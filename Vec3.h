...
public:
	...
	Vec3 getNormSafe() const { Vec3 v(*this);  v.normalizeSafe(); return v; }
...
template <class T>
inline T Vec3<T>::normalizeSafe()
{
	T len = length();
	
	if (len > std::numeric_limits<T>::epsilon()) // ? std::minT? smallest number you can divide by..
	{
		for (size_t i=0; i<dim; ++i)
		{
			m_vec[i] = m_vec[i]/len;
